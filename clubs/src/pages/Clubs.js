            ...
            <div className='left flex-1 bg-teeth'>
                <div className='flex flex-col items-center justify-center h-full'>
                    ...
                    <form className='flex flex-col items-center justify-center'>
                        <input className='w-80 h-10 p-2 m-2 rounded-md border' type='text' placeholder='Search for a club' onChange={handleSearch} />
                    </form>
                </div>
            </div>
            <div className='right flex-1 flex items-center justify-center'>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4'>
                    {(searchedClubs.length > 0 ? searchedClubs : allClubs).map((club) => (
                        <div key={club.id} className='bg-white rounded-md p-4 flex flex-col items-center justify-center border'>
                            ...
                            <Link to="https://docs.google.com/forms/d/1I575FthbthitSCIq5LV8oUxf6qXatOcS4zXNvwFYzF8/prefill" ><button className='w-32 h-10 m-2 p-2 bg-indigo-950 text-white rounded-md'  >Join</button></Link>
                        </div>
                    ))}
